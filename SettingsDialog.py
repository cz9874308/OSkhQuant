...
本模块提供系统参数配置界面，允许用户自定义各项系统设置。

核心功能
--------
- **基本参数**：无风险收益率、日志显示设置、最大日志行数
- **账户设置**：账户名称、账户类型配置
- **路径配置**：miniQMT 客户端路径、数据路径
- **股票列表**：更新成分股列表
- **版本信息**：显示当前软件版本和构建信息

配置项说明
----------
- **无风险收益率**：用于计算夏普比率等指标（0-1之间）
- **延迟显示日志**：策略运行期间延迟日志显示，提升性能
- **最大日志行数**：限制日志显示行数（100-100000）
- **初始化行情数据**：启动时是否初始化行情数据连接
