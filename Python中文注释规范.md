# Python 中文注释规范

## 📋 概述

本文档参考 Rust 中文注释规范，结合 Python 语言特性，总结的 Python 中文注释规范，旨在提供清晰、统一、易读的代码文档标准。

## 🎯 注释类型

Python 支持三种类型的注释：

| 注释类型              | 语法      | 用途                           | 是否生成文档 |
| --------------------- | --------- | ------------------------------ | ------------ |
| **模块级文档字符串**  | `"""` 或 `'''` | 描述整个模块的功能             | ✅ 是        |
| **公共 API 文档字符串** | `"""` 或 `'''` | 描述类、函数、方法等           | ✅ 是        |
| **行内注释**          | `#`       | 解释代码逻辑                   | ❌ 否        |

---

## 📝 模块级文档字符串

### 使用场景

用于描述整个模块（`.py` 文件）的功能、设计理念和使用方式。通常放在文件开头，在 `import` 语句之前。

### 基本格式

```python
"""
模块简要描述

模块的详细说明，可以包含多行。

核心概念
--------

- **概念1**: 说明
- **概念2**: 说明

使用方式
--------

1. 步骤一
2. 步骤二

注意事项
--------

重要提示或警告信息。
"""
```

### 示例

```python
"""
服务依赖注入模块

本模块实现了运行时依赖注入功能，允许服务自动注入依赖的组件和配置。

核心概念
--------

- **Service**: 特殊的组件，支持通过字段属性自动注入依赖
- **ServiceRegistry**: 服务注册表，负责将服务注册到应用中
- **自动注入**: 在应用启动时自动发现并安装所有服务

使用方式
--------

1. 定义服务类，继承 `Service` 基类
2. 使用 `@inject` 装饰器标记需要注入的字段
3. 框架会在应用启动时自动发现并注册所有服务

依赖解析
--------

服务注册时会自动解析依赖关系，确保依赖的服务先于被依赖的服务注册。
"""

from typing import Optional, Dict, Any
```

### 推荐章节结构

- **概述**：模块的核心功能和目的
- **核心概念**：关键术语和概念解释
- **使用方式**：基本使用步骤
- **工作原理**：内部实现机制（如需要）
- **注意事项**：重要提示或限制

---

## 📚 公共 API 文档字符串

### 使用场景

用于描述公共类、函数、方法、属性等公共 API。

### 基本格式（Google 风格）

```python
def function_name(param1: Type1, param2: Type2) -> ReturnType:
    """功能简要描述

    详细说明，可以包含多行。

    Args:
        param1: 参数说明
        param2: 参数说明

    Returns:
        返回值说明

    Raises:
        ExceptionType: 可能的错误情况说明

    Example:
        >>> function_name(value1, value2)
        result

    Note:
        重要提示或限制。
    """
    pass
```

### 基本格式（NumPy 风格）

```python
def function_name(param1: Type1, param2: Type2) -> ReturnType:
    """功能简要描述

    详细说明，可以包含多行。

    Parameters
    ----------
    param1 : Type1
        参数说明
    param2 : Type2
        参数说明

    Returns
    -------
    ReturnType
        返回值说明

    Raises
    ------
    ExceptionType
        可能的错误情况说明

    Examples
    --------
    >>> function_name(value1, value2)
    result

    Notes
    -----
    重要提示或限制。
    """
    pass
```

### 示例：函数注释（Google 风格）

```python
def auto_inject_services(app: AppBuilder) -> None:
    """自动注入所有服务到应用中

    查找所有已注册的服务并将它们安装到应用中。

    工作原理
    --------

    1. 收集所有通过装饰器注册的服务
    2. 循环安装服务，直到所有服务都安装成功：
       - 尝试安装每个待安装的服务
       - 如果成功，从待安装列表移除
       - 如果失败（可能是依赖未满足），保留在待安装列表
    3. 如果一轮中没有进展，说明存在循环依赖或缺少依赖，抛出异常

    Args:
        app: 应用构建器实例

    Raises:
        DependencyError: 如果存在循环依赖或缺少依赖

    Example:
        >>> app = AppBuilder()
        >>> auto_inject_services(app)
        >>> # 现在所有服务都已安装

    Note:
        使用拓扑排序算法解析服务依赖关系。
    """
    pass
```

### 示例：类注释

```python
class Service:
    """Service 基类：支持依赖注入的特殊组件

    Service 是一种特殊的组件，可以通过字段属性自动注入依赖的组件和配置。

    实现要求
    --------

    - 必须继承 `Service` 基类
    - 必须实现 `build()` 方法，用于从注册表构建服务实例

    使用示例
    --------

    ```python
    class UserService(Service):
        def __init__(self, db: Database):
            self.db = db

        def build(registry: Registry) -> 'UserService':
            db = registry.get_component(Database)
            return UserService(db)
    ```

    工作原理
    --------

    1. 子类通过 `@service` 装饰器自动注册
    2. 服务在应用启动时通过 `inventory` 自动发现
    3. 应用构建时，`auto_inject_services()` 会遍历所有注册的服务
    4. 按照依赖顺序安装服务，确保依赖的服务先安装

    Attributes:
        registry: 服务注册表，用于获取依赖组件
    """
    pass
```

### 示例：方法注释

```python
def build(self, registry: Registry) -> 'Service':
    """从注册表构建服务组件

    此方法由子类实现，会：
    1. 从组件注册表获取标记为 `@inject` 的字段
    2. 从配置注册表获取标记为 `@inject_config` 的字段
    3. 构建并返回服务实例

    Args:
        registry: 组件和配置注册表

    Returns:
        构建的服务实例

    Raises:
        ComponentNotFoundError: 如果依赖的组件未找到
        ConfigNotFoundError: 如果依赖的配置未找到
    """
    pass
```

### 推荐章节结构

根据 API 的复杂度，可以包含以下章节：

- **概述**：功能简要描述（必需）
- **工作原理**：内部实现机制（如需要）
- **Args/Parameters**：函数参数说明
- **Returns**：返回值说明
- **Raises/Exceptions**：可能的错误情况
- **Example/Examples**：代码示例（强烈推荐）
- **Note/Notes**：重要提示或限制
- **Attributes**：类属性说明（对于类）
- **Methods**：类方法说明（对于类）

---

## 💬 行内注释

### 使用场景

用于解释代码逻辑、算法步骤、关键决策等。

### 基本格式

```python
# 简洁说明代码意图
result = calculate()

# 多行注释可以这样写：
# 第一行说明
# 第二行说明
```

### 示例

```python
# 收集所有服务注册器
registrars = [r for r in ServiceRegistry.get_all()]

# 循环安装服务，直到所有服务都安装成功
pending = list(registrars)
installed = 0

while pending:
    next_pending = []
    progress_made = False

    # 尝试安装每个待安装的服务
    for registrar in pending:
        try:
            registrar.install(app)
            # 安装成功
            installed += 1
            progress_made = True
        except DependencyError:
            # 安装失败（可能是依赖未满足），等待下一轮
            next_pending.append(registrar)

    # 如果一轮中没有进展，说明存在循环依赖或缺少依赖
    if not progress_made and next_pending:
        raise DependencyError("循环依赖或缺少依赖")
    
    pending = next_pending
```

### 注释原则

- **简洁明了**：注释应该简洁，避免冗长
- **解释"为什么"**：优先解释代码的意图和原因，而非"是什么"
- **避免重复**：不要重复代码本身已经表达清楚的内容
- **及时更新**：代码修改时，同步更新相关注释

---

## 📖 Markdown 格式支持

Python 文档字符串支持完整的 Markdown 语法（在大多数文档生成工具中）。

### 常用 Markdown 元素

#### 标题

```python
"""
# 一级标题
## 二级标题
### 三级标题
"""
```

#### 列表

```python
"""
- 无序列表项 1
- 无序列表项 2

1. 有序列表项 1
2. 有序列表项 2
"""
```

#### 强调

```python
"""
这是 **粗体** 文本
这是 *斜体* 文本
这是 ``代码`` 文本
"""
```

#### 代码块

```python
"""
```python
# 示例代码
result = function_name(value1, value2)
```
"""
```

#### 表格

```python
"""
| 列1 | 列2 | 列3 |
|-----|-----|-----|
| 值1 | 值2 | 值3 |
"""
```

#### 链接

```python
"""
查看 [Python 官方文档](https://docs.python.org/)
参考 `Service` 类
"""
```

---

## 🎨 类型注解

### 基本用法

Python 3.5+ 支持类型注解，建议在函数签名中使用：

```python
from typing import Optional, List, Dict, Any, Union

def get_user_info(
    user_id: int,
    include_profile: bool = False
) -> Optional[Dict[str, Any]]:
    """获取用户信息

    Args:
        user_id: 用户 ID
        include_profile: 是否包含用户资料，默认为 False

    Returns:
        用户信息字典，如果用户不存在则返回 None
    """
    pass
```

### 复杂类型注解

```python
from typing import List, Dict, Optional, Union, Tuple

def process_data(
    items: List[Dict[str, Union[int, str]]],
    filters: Optional[List[str]] = None
) -> Tuple[List[Dict], int]:
    """处理数据项

    Args:
        items: 数据项列表，每个项是包含整数或字符串的字典
        filters: 可选的过滤器列表

    Returns:
        处理后的数据列表和总数
    """
    pass
```

---

## 🎯 复杂函数的人性化注释指南

对于复杂的函数，需要编写更加人性化、通俗易懂的文档注释，帮助读者快速理解函数的目的、工作原理和使用方式。

### 编写原则

1. **用通俗语言解释**：避免过于技术化的术语，用日常语言解释概念
2. **提供背景信息**：说明函数解决什么问题，为什么需要这个函数
3. **分步骤说明**：将复杂过程分解为清晰的步骤
4. **使用类比**：用熟悉的概念类比复杂的概念
5. **提供实际场景**：说明在什么情况下使用这个函数
6. **解释关键决策**：说明为什么这样实现，有什么考虑

### 对比示例：不好的注释 vs 好的注释

#### ❌ 不好的注释（过于技术化，难以理解）

```python
def auto_inject_services(app: AppBuilder) -> None:
    """自动注入所有服务

    遍历 inventory 收集的 ServiceRegistry，通过拓扑排序解析依赖，
    循环调用 install_service 直到所有服务安装完成或检测到循环依赖。

    Args:
        app: AppBuilder 实例
    """
    pass
```

**问题**：

- 直接使用技术术语（inventory、拓扑排序），没有解释
- 没有说明为什么需要这个函数
- 没有解释"循环依赖"是什么，为什么会出现
- 缺少使用场景说明

#### ✅ 好的注释（人性化、通俗易懂）

```python
def auto_inject_services(app: AppBuilder) -> None:
    """自动注入所有服务到应用中

    这个函数的作用就像"自动装配工"，它会自动找到所有需要安装的服务，
    并按照正确的顺序把它们安装到应用中。

    为什么需要这个函数？
    --------------------

    在应用中，服务之间可能存在依赖关系。比如：
    - 用户服务需要数据库连接
    - 订单服务需要用户服务和数据库连接

    如果先安装订单服务，再安装用户服务，就会出错，因为订单服务依赖的用户服务还不存在。
    这个函数会自动解决这个问题，确保依赖的服务总是先于被依赖的服务安装。

    工作原理（简单理解）
    ------------------

    想象一下整理书架的过程：

    1. **收集所有书**：找到所有需要安装的服务（就像收集所有要放上书架的书）
    2. **尝试摆放**：尝试安装每个服务
       - 如果服务的依赖都已经安装好了，就安装这个服务（就像书的前置书籍都已经在书架上了）
       - 如果依赖还没安装，就暂时跳过，等下一轮再试（就像前置书籍还没到，先放一边）
    3. **重复直到完成**：重复步骤 2，直到所有服务都安装成功
    4. **检测问题**：如果一轮下来没有任何进展，说明存在循环依赖或缺少依赖，抛出异常

    实际使用场景
    -----------

    通常在应用启动时调用，一次性安装所有服务：

    ```python
    app = AppBuilder()

    # 配置数据库、日志等基础组件
    app.add_component(database)
    app.add_component(logger)

    # 自动安装所有服务（用户服务、订单服务等）
    auto_inject_services(app)

    # 现在所有服务都可以使用了
    app.run()
    ```

    可能遇到的问题
    ------------

    循环依赖
    ^^^^^^^

    如果服务 A 依赖服务 B，服务 B 又依赖服务 A，就会形成循环依赖。
    这种情况下，函数会抛出 `DependencyError` 异常，提示你检查服务的依赖关系。

    缺少依赖
    ^^^^^^^

    如果服务依赖的组件或配置还没有添加到应用中，函数也会抛出异常。
    确保在调用此函数之前，所有必需的组件都已经添加到应用中。

    Args:
        app: 应用构建器，用于安装服务

    Raises:
        DependencyError: 如果存在循环依赖或缺少依赖

    Example:
        >>> app = AppBuilder()
        >>> app.add_component(create_database_pool())
        >>> auto_inject_services(app)
        >>> print("所有服务安装成功！")
    """
    pass
```

**优点**：

- 用"自动装配工"类比，形象易懂
- 解释了为什么需要这个函数（依赖关系问题）
- 用"整理书架"类比工作原理，容易理解
- 提供了实际使用场景
- 说明了可能遇到的问题和解决方案

### 复杂函数注释模板

```python
def complex_function(param1: Type1, param2: Type2) -> ReturnType:
    """[函数功能简要描述]

    [用一句话或一个类比说明这个函数是做什么的，让读者快速理解]

    为什么需要这个函数？
    --------------------

    [说明这个函数解决什么问题，为什么需要它]

    工作原理（简单理解）
    ------------------

    [用通俗语言或类比解释工作原理，可以分步骤说明]

    1. [步骤一：用日常语言描述]
    2. [步骤二：用日常语言描述]
    3. [步骤三：用日常语言描述]

    实际使用场景
    -----------

    [说明在什么情况下使用这个函数，提供实际场景]

    ```python
    # 使用示例
    ```

    可能遇到的问题
    ------------

    问题一
    ^^^^^

    [解释问题是什么，如何避免或解决]

    问题二
    ^^^^^

    [解释问题是什么，如何避免或解决]

    Args:
        param1: [参数说明，用通俗语言]

    Returns:
        [返回值说明，用通俗语言]

    Raises:
        ExceptionType: [异常说明]

    Example:
        >>> # 完整的使用示例
    """
    pass
```

### 更多人性化注释示例

#### 示例 1：依赖解析函数

```python
def resolve_dependencies(services: List[Service]) -> List[Service]:
    """解析服务依赖关系，确定安装顺序

    这个函数就像一个"任务调度器"，它会分析所有服务之间的依赖关系，
    然后告诉你应该按照什么顺序安装这些服务。

    为什么需要这个函数？
    --------------------

    想象你要做一顿饭：
    - 做菜需要先洗菜
    - 炒菜需要先切菜
    - 切菜需要先洗菜

    如果你不按顺序来，比如先炒菜再洗菜，就会出问题。
    服务安装也是一样，必须按照依赖顺序来。

    工作原理
    --------

    这个函数使用"拓扑排序"算法（就像整理任务清单一样）：

    1. **建立依赖图**：画出所有服务之间的依赖关系
       - 用户服务 → 依赖 → 数据库连接
       - 订单服务 → 依赖 → 用户服务、数据库连接

    2. **找到起点**：找出不依赖任何其他服务的服务（就像找到可以立即开始的任务）

    3. **逐步扩展**：安装这些服务后，再安装依赖它们的服务

    4. **检测循环**：如果发现循环依赖（A 依赖 B，B 又依赖 A），抛出异常

    实际使用场景
    -----------

    通常在应用启动时，在安装服务之前调用，确保安装顺序正确：

    ```python
    # 解析依赖关系
    install_order = resolve_dependencies(services)

    # 按照顺序安装
    for service in install_order:
        service.install(app)
    ```

    Args:
        services: 所有需要安装的服务列表

    Returns:
        按安装顺序排列的服务列表

    Raises:
        CircularDependencyError: 如果存在循环依赖

    Example:
        >>> services = [user_service, order_service, db_service]
        >>> order = resolve_dependencies(services)
        >>> # order = [db_service, user_service, order_service]
    """
    pass
```

#### 示例 2：缓存管理函数

```python
def get_cached_user(key: str, ttl: int = 3600) -> Optional[User]:
    """智能缓存管理器：自动决定何时更新缓存

    这个函数就像一个"智能管家"，它会自动判断缓存是否过期，
    如果过期了就更新，没过期就直接使用，让你不用每次都手动检查。

    为什么需要这个函数？
    --------------------

    假设你有一个用户信息缓存：
    - 如果每次都去数据库查询，会很慢
    - 如果一直用缓存，数据可能过期

    这个函数帮你自动处理这个平衡：需要时更新，不需要时用缓存。

    工作原理
    --------

    就像检查冰箱里的食物是否过期：

    1. **检查缓存是否存在**：看看冰箱里有没有这个食物
    2. **检查是否过期**：看看食物的保质期过了没有
       - 如果没过期：直接使用（返回缓存的数据）
       - 如果过期了：重新获取（去超市买新的，更新缓存）
    3. **返回结果**：给你最新的数据

    实际使用场景
    -----------

    适用于需要定期更新的数据，比如用户信息、配置信息等：

    ```python
    # 获取用户信息（自动处理缓存）
    user = get_cached_user(user_id)

    # 如果缓存过期，会自动从数据库获取最新数据
    # 如果缓存有效，直接返回缓存数据，速度很快
    ```

    缓存策略
    -------

    - **时间过期**：缓存超过指定时间后自动过期
    - **版本过期**：如果数据版本号变化，缓存自动失效
    - **手动刷新**：可以手动调用刷新缓存

    Args:
        key: 缓存的键（就像食物的名称）
        ttl: 缓存有效期（秒），就像食物的保质期，默认 3600 秒

    Returns:
        缓存的值（可能是旧的，也可能是刚更新的），如果获取失败返回 None

    Example:
        >>> user = get_cached_user("user_123", ttl=7200)
        >>> print(user.name)
    """
    pass
```

### 人性化注释检查清单

编写复杂函数注释时，检查以下内容：

- [ ] **有通俗易懂的类比**：用熟悉的概念解释复杂概念
- [ ] **解释了"为什么"**：说明函数解决什么问题
- [ ] **分步骤说明**：将复杂过程分解为清晰步骤
- [ ] **提供使用场景**：说明什么时候使用这个函数
- [ ] **说明可能的问题**：提前告知可能遇到的问题
- [ ] **避免过度技术化**：用日常语言，少用专业术语
- [ ] **提供完整示例**：包含实际可运行的代码示例
- [ ] **解释关键决策**：说明为什么这样实现

### 常见技术术语的通俗解释

| 技术术语     | 通俗解释                                                         | 使用场景 |
| ------------ | ---------------------------------------------------------------- | -------- |
| **拓扑排序** | 就像整理任务清单，找出先做什么后做什么                           | 依赖解析 |
| **依赖注入** | 就像点外卖，你不需要知道怎么做，只需要说你要什么，系统会自动给你 | 服务管理 |
| **缓存**     | 就像把常用的东西放在手边，需要时直接拿，不用每次都去远处取       | 性能优化 |
| **异步**     | 就像同时做多件事，不用等一件事做完再做另一件                     | 并发处理 |
| **装饰器**   | 就像给函数"穿衣服"，在不改变函数本身的情况下给它添加新功能         | 功能增强 |
| **生成器**   | 就像"按需生产"，需要时才生成数据，节省内存                       | 大数据处理 |
| **上下文管理器** | 就像"自动开关门"，进入时自动打开，退出时自动关闭                 | 资源管理 |

---

## 🏷️ 变量命名规范

### 概述

Python 遵循 PEP 8 命名约定，不同类型的标识符使用不同的命名风格。遵循这些约定可以让代码更易读、更符合 Python 社区习惯。

### 命名风格对照表

| 标识符类型       | 命名风格               | 示例                           | 说明                 |
| ---------------- | ---------------------- | ------------------------------ | -------------------- |
| **类**           | `PascalCase`           | `UserService`, `Middlewares`   | 首字母大写的驼峰命名 |
| **异常**         | `PascalCase`           | `ValueError`, `DependencyError` | 首字母大写的驼峰命名 |
| **类型别名**     | `PascalCase`           | `ConfigMap`, `UserDict`        | 首字母大写的驼峰命名 |
| **函数**         | `snake_case`           | `auto_inject_service`, `build` | 小写下划线命名       |
| **变量**         | `snake_case`           | `user_name`, `app_builder`     | 小写下划线命名       |
| **方法**         | `snake_case`           | `get_user`, `process_data`     | 小写下划线命名       |
| **模块**         | `snake_case`           | `service`, `plugin`            | 小写下划线命名       |
| **包**           | `snake_case`           | `panda_common`, `panda_data`   | 小写下划线命名       |
| **常量**         | `SCREAMING_SNAKE_CASE` | `MAX_SIZE`, `DEFAULT_TIMEOUT`  | 全大写下划线命名     |
| **私有变量/方法** | `_snake_case` 或 `__snake_case` | `_internal`, `__private` | 单下划线或双下划线前缀 |

---

### 类命名

#### 基本规则

- 使用 `PascalCase` 命名
- 名称应该是名词或名词短语
- 避免缩写，除非是广泛使用的（如 `HTTP`, `URL`, `ID`）

#### 示例

```python
class UserService:
    """用户服务类"""
    pass

class Middlewares:
    """中间件配置类"""
    pass

class HttpRequestConfig:
    """HTTP 请求配置类"""
    pass
```

---

### 函数和变量命名

#### 基本规则

- 使用 `snake_case` 命名
- 函数名应该是动词或动词短语
- 变量名应该是名词或名词短语
- 布尔变量使用 `is_`, `has_`, `should_`, `can_` 等前缀

#### 函数命名示例

```python
def auto_inject_services(app: AppBuilder) -> None:
    """自动注入所有服务"""
    pass

def build_service(registry: Registry) -> Service:
    """从注册表构建服务组件"""
    pass

def check_user_exists(user_id: int) -> bool:
    """检查用户是否存在"""
    pass

def get_user_info(user_id: int) -> Optional[User]:
    """获取用户信息"""
    pass
```

#### 变量命名示例

```python
def auto_inject_services(app: AppBuilder) -> None:
    """自动注入所有服务"""
    # 收集所有服务注册器
    registrars = [r for r in ServiceRegistry.get_all()]
    
    total = len(registrars)
    pending = list(registrars)
    installed = 0
    progress_made = False
    
    # 循环安装服务
    while pending:
        next_pending = []
        
        # 尝试安装每个待安装的服务
        for registrar in pending:
            try:
                registrar.install(app)
                installed += 1
                progress_made = True
            except DependencyError:
                next_pending.append(registrar)
        
        pending = next_pending
```

#### 布尔变量命名

```python
# 使用 is_ 前缀
is_active = True
is_verified = False
is_empty = len(items) == 0

# 使用 has_ 前缀
has_permission = check_permission()
has_children = node.has_children()

# 使用 should_ 前缀
should_retry = error.is_retryable()
should_log = log_level >= logging.INFO

# 使用 can_ 前缀
can_edit = user.has_permission(Permission.EDIT)
can_delete = user.has_permission(Permission.DELETE)
```

---

### 常量和私有变量命名

#### 基本规则

- 常量使用 `SCREAMING_SNAKE_CASE`（全大写下划线）
- 私有变量使用 `_snake_case`（单下划线前缀）
- 类私有变量使用 `__snake_case`（双下划线前缀）

#### 示例

```python
# 常量
MAX_RETRY_COUNT = 3
DEFAULT_TIMEOUT = 30
MAX_REQUEST_BODY_SIZE = 10 * 1024 * 1024  # 10MB

# 模块级私有变量
_internal_cache = {}

# 类私有变量
class Service:
    __private_method = None  # 名称会被改写为 _Service__private_method
    _protected_method = None  # 约定为受保护的方法
```

---

### 模块和包命名

#### 基本规则

- 使用 `snake_case` 命名
- 模块名应该是名词或名词短语
- 避免缩写，除非是广泛使用的

#### 示例

```python
# 文件：service.py
"""
服务依赖注入模块
"""

# 文件：config/registry.py
"""
配置注册表模块
"""

# 文件：error/handler.py
"""
错误处理模块
"""
```

---

### 命名规范检查清单

#### 类和类型

- [ ] 使用 `PascalCase` 命名
- [ ] 名称清晰描述类型用途
- [ ] 避免不必要的缩写

#### 函数和变量

- [ ] 使用 `snake_case` 命名
- [ ] 函数名是动词或动词短语
- [ ] 变量名是名词或名词短语
- [ ] 布尔变量使用 `is_`, `has_`, `should_` 等前缀

#### 常量

- [ ] 使用 `SCREAMING_SNAKE_CASE` 命名
- [ ] 名称清晰描述常量用途

#### 模块和包

- [ ] 使用 `snake_case` 命名
- [ ] 模块名清晰描述模块功能

#### 通用要求

- [ ] 遵循 PEP 8 命名约定
- [ ] 名称具有描述性，避免单字母变量（除非是循环变量）
- [ ] 避免使用 Python 关键字作为标识符
- [ ] 保持命名一致性

---

## ✅ 注释规范检查清单

### 模块级注释

- [ ] 包含模块的简要描述
- [ ] 说明核心概念（如需要）
- [ ] 提供使用方式或示例（如需要）
- [ ] 说明注意事项或限制（如需要）

### 公共 API 注释

- [ ] 包含功能简要描述
- [ ] 说明参数（如有）
- [ ] 说明返回值
- [ ] 说明异常处理（如可能出错）
- [ ] 提供使用示例（强烈推荐）
- [ ] 说明实现要求（对于抽象基类）

### 行内注释

- [ ] 解释复杂逻辑
- [ ] 说明关键决策
- [ ] 避免重复代码本身

### 通用要求

- [ ] 使用中文编写注释
- [ ] 使用 Markdown 格式（在文档字符串中）
- [ ] 保持注释与代码同步
- [ ] 代码示例可以运行（如使用 `>>>` 提示符）

---

## 🎨 注释风格示例

### 完整示例：模块 + 类 + 方法

```python
"""
服务依赖注入模块

本模块实现了运行时依赖注入功能，允许服务自动注入依赖的组件和配置。
"""

from typing import Optional, Dict, Any
from abc import ABC, abstractmethod


class Service(ABC):
    """Service 基类：支持依赖注入的特殊组件

    Service 是一种特殊的组件，可以通过字段属性自动注入依赖的组件和配置。

    实现要求
    --------

    - 必须继承 `Service` 基类
    - 必须实现 `build()` 方法，用于从注册表构建服务实例

    使用示例
    --------

    ```python
    class UserService(Service):
        def build(registry: Registry) -> 'UserService':
            db = registry.get_component(Database)
            return UserService(db)
    ```
    """

    @abstractmethod
    def build(self, registry: 'Registry') -> 'Service':
        """从注册表构建服务组件

        此方法由子类实现，会：
        1. 从组件注册表获取标记为 `@inject` 的字段
        2. 从配置注册表获取标记为 `@inject_config` 的字段
        3. 构建并返回服务实例

        Args:
            registry: 组件和配置注册表

        Returns:
            构建的服务实例

        Raises:
            ComponentNotFoundError: 如果依赖的组件未找到
        """
        pass


def auto_inject_services(app: 'AppBuilder') -> None:
    """自动注入所有服务

    查找所有已注册的服务并将它们安装到应用中。

    Args:
        app: 应用构建器

    Raises:
        DependencyError: 如果存在循环依赖或缺少依赖

    Example:
        >>> app = AppBuilder()
        >>> auto_inject_services(app)
    """
    # 收集所有服务注册器
    registrars = [r for r in ServiceRegistry.get_all()]
    
    # 循环安装服务，直到所有服务都安装成功
    pending = list(registrars)
    installed = 0
    
    while pending:
        next_pending = []
        progress_made = False
        
        # 尝试安装每个待安装的服务
        for registrar in pending:
            try:
                registrar.install(app)
                installed += 1
                progress_made = True
            except DependencyError:
                # 安装失败（可能是依赖未满足），等待下一轮
                next_pending.append(registrar)
        
        # 如果一轮中没有进展，说明存在循环依赖或缺少依赖
        if not progress_made and next_pending:
            raise DependencyError("循环依赖或缺少依赖")
        
        pending = next_pending


# 常量
MAX_RETRY_COUNT = 3
DEFAULT_TIMEOUT = 30
```

---

## 📚 相关资源

- [PEP 8 -- Style Guide for Python Code](https://www.python.org/dev/peps/pep-0008/)
- [PEP 257 -- Docstring Conventions](https://www.python.org/dev/peps/pep-0257/)
- [Google Python Style Guide](https://google.github.io/styleguide/pyguide.html)
- [NumPy Docstring Guide](https://numpydoc.readthedocs.io/en/latest/format.html)
- [Sphinx Documentation](https://www.sphinx-doc.org/)

---

## 🔄 版本历史

- **v1.0.0** (2024-12-11): 初始版本，参考 Rust 中文注释规范，适配 Python 语言特性

---

**💡 提示**：良好的注释是代码可维护性的重要保障。遵循本规范，让代码更易读、易懂、易维护。
